...
# dir making
docker exec -it star_rsem_pipe /bin/sh -c "mkdir -p /data/output_dir/QC; mkdir -p /data/output_dir/STAR;mkdir -p /data/output_dir/RSEM;chmod -R 777 /data/output_dir;apt-get update && apt-get install -y pigz"

## RUN pipeline
# trim_galore CMD
if [ ${type} = "fastq" ];then
    echo "Trim Galore RUN!!!!!!!!"
    docker exec trim_galore /bin/sh -c "/data/RNA_script/pipe_script/trim_galore_pipe_single.sh -i /data/${indir} -o /data/output_dir"
fi
...
docker stop star_rsem_pipe trim_galore